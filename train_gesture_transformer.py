...
def train(config):
    ...
    then instantiates a `GestureTrainer` object to handle the training process according to the specified configuration.
    ...
    2. Instantiates a `GestureTrainer` object, passing in the `config` and `logger`.
    3. Calls the `train` method of `GestureTrainer` to begin the training process.
    ...
    Notes:
    ------
    - The `Log` and `GestureTrainer` classes must be predefined. `Log` should support `info` and other logging methods,
      while `GestureTrainer` should implement a `train` method to conduct the actual training.
    - `train.log` will be saved in the `output_path` directory as specified in the `config`.
    """

    # Set up logging
    logger = Log(log_file=os.path.join(config.get("output_path"), "train.log"), name="Main::", verbose=config.get("verbose"))

    logger.info("Main initialin trainer!")

    # Instantiate trainer
    trainer = GestureTrainer(config, logger=logger.set_logname(GestureTrainer))
...
def main():
    ...
    # use_xla_accelerator = bool(config.get("use_xla_accelerator"))
    # if use_xla_accelerator:
    #     os.environ['TF_XLA_FLAGS'] = "--tf_xla_cpu_global_jit"
    #     os.environ['cpu_global_jit'] = True
