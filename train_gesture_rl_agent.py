...
from trainer import TGGMTSearchEnvTrainer
...
def train(config):
    ...
    then instantiates a `TGGMTSearchEnvTrainer` object to handle the training process according to the specified configuration.
    ...
    2. Instantiates a `TGGMTSearchEnvTrainer` object, passing in the `config` and `logger`.
    3. Calls the `train` method of `TGGMTSearchEnvTrainer` to begin the training process.
    ...
    Notes:
    ------
    - The `Log` and `TGGMTSearchEnvTrainer` classes must be predefined. `Log` should support `info` and other logging methods,
      while `TGGMTSearchEnvTrainer` should implement a `train` method to conduct the actual training.
    ...
    # Instantiate trainer
    trainer = TGGMTSearchEnvTrainer(config, logger=logger)
    # Start training
    trainer.train()
