...
from trainer import GestureTuner
...
def tune(config):
    ...
    then instantiates a `Gesturetuneer` object to handle the tuning process according to the specified configuration.
    ...
    2. Instantiates a `Gesturetuneer` object, passing in the `config` and `logger`.
    3. Calls the `tune` method of `Gesturetuneer` to begin the tuning process.
    ...
    Notes:
    ------
    - The `Log` and `Gesturetuneer` classes must be predefined. `Log` should support `info` and other logging methods,
      while `Gesturetuneer` should implement a `tune` method to conduct the actual tuning.
    - `tune.log` will be saved in the `output_path` directory as specified in the `config`.
    """

    # Set up logging
    logger = Log(log_file=os.path.join(config.get("output_path"), "tuner.log"), verbose=config.get("verbose"))

    # Instantiate tuneer
    tuner = GestureTuner(config, logger=logger)
